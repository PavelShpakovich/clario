...
export const POST = withApiHandler(async (req) => {
  ...
  if (existingUser) {
    ...
    await sendVerificationEmail({ userId: existingUser.id, email, source });
  ...
  const { data, error } = await supabaseAdmin.auth.admin.createUser({
    ...
    password,
    email_confirm: false,
    user_metadata: { source: source ?? 'web' },
  });
  ...
  await sendVerificationEmail({ userId: data.user.id, email, source });
