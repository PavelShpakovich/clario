...
describe('GET /api/auth/confirm', () => {
  ...
  beforeAll(async () => {
    const mod = await import('@/app/api/auth/confirm/route');
    handler = mod.GET;
  });

  beforeEach(() => {
    jest.clearAllMocks();
    (findEmailVerificationToken as jest.Mock).mockResolvedValue({
      user_id: 'user-123',
      email: 'user@example.com',
    });
  ...
  it('confirms the user and redirects to onboarding login on success', async () => {
    const res = await handler(makeGetRequest('/api/auth/confirm?token=test-token'));

    expect(findEmailVerificationToken).toHaveBeenCalledWith('test-token');
    expect(consumeEmailVerificationToken).toHaveBeenCalledWith('test-token');
    expect(supabaseAdmin.auth.admin.updateUserById).toHaveBeenCalledWith('user-123', {
      email_confirm: true,
    });
    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toContain('/login?callbackUrl=/onboarding');
  });

  it('redirects successfully when a parallel request already confirmed the user', async () => {
    (consumeEmailVerificationToken as jest.Mock).mockResolvedValue(null);
    ...
    const res = await handler(makeGetRequest('/api/auth/confirm?token=test-token&source=mobile'));

    expect(consumeEmailVerificationToken).toHaveBeenCalledWith('test-token');
    expect(supabaseAdmin.auth.admin.updateUserById).not.toHaveBeenCalled();
    expect(res.status).toBe(307);
    expect(res.headers.get('location')).toBe('clario://login?verified=true');
  });
});
